...
void loop() {
    ...
    for (size_t i = 0; i < ids.size(); ++i) {
        ...
        sprintf(log_msg, "%3d: %c%4d.%01d,", ids[i], current < 0 ? '-' : ' ', abs(int(current)),
                int(int(1000 * abs(current)) % 1000 / 100));
        DEBUG_SERIAL.print(log_msg);
    }
    DEBUG_SERIAL.print('\n');
    ...
    for (size_t i = 0; i < ids.size(); ++i) {
        ...
        sprintf(log_msg, "%3d:  %4d.%01d,", ids[i], abs(int(temperature)),
                int(int(1000 * abs(temperature)) % 1000 / 100));
        DEBUG_SERIAL.print(log_msg);
    }
    DEBUG_SERIAL.print('\n');
