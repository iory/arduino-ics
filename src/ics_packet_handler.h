...
public:
    explicit ICSPacketHandler(uint8_t device_id)
        : buffer_pos_(0), packet_ready_(false), id_changed_(false) {
        setId(device_id);
        id_changed_ = false;
    }
    ...
    bool processByte(uint8_t byte) {
        if (buffer_pos_ == 0) {
            if (!isValidHeader(byte) || (((byte & 0xE0) != 0xE0) && (byte & 0x1F) != getId())) {
                return false;
            }
        }
...
protected:
    virtual bool isValidHeader(uint8_t byte) const { return (byte & 0xE0) >= 0x20; }
