...
IcsHardSerialClass::IcsHardSerialClass(
        HardwareSerial* serial, long baudRate, int timeout, int enPin, int openDrainTxPin)
    : serial_(serial),
      enPin_(enPin),
      baudRate_(baudRate),
      timeout_(timeout),
      openDrainTxPin_(openDrainTxPin),
      mutex_() {}
...
bool IcsHardSerialClass::synchronizeImpl(uint8_t* txBuffer,
                                         size_t txLength,
                                         uint8_t* rxBuffer,
                                         size_t rxLength) {
    if (!serial_) {
...
bool IcsHardSerialClass::synchronize(uint8_t* txBuffer,
                                     size_t txLength,
                                     uint8_t* rxBuffer,
                                     size_t rxLength) {
    mutex_.lock();
